...
We will use notation of \cite{Laskowski}, in particular things like $\K$, $\delta(\A/\B)$, $X_m(\A)$, $\S$, maximal embedding, $\Delta_\A(x)$, $\Psi_{\A,\B}(x)$ etc. However we will give a different definition of $Y(\ldots)$. When we write formulas $\theta(x,y)$ we may have $x,y$ to be tuples.
...
\end{align*}
for some finite subsets $\Phi, \Gamma$ of $\K$. We need to eliminate quantifier $\exists y \theta(x, y)$.
