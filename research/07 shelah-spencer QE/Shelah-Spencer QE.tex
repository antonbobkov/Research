...
We will use notation of \cite{Laskowski}, in particular things like $\K$, $\delta(\A/\B)$, $X_m(\A)$, $\S$, maximal embedding, $\Psi_{\A,\B}(\vec x)$ etc. However we will give a different definition of $Y(\ldots)$.
...
Following proof of 5.6 in \cite{Laskowski}, we have a formula $\theta(\vec x, \vec y)$, some $\A, \B \in \K$ with $\theta(\vec x, \vec y) \vdash $ 
