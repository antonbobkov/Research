...
Following proof of 5.6 in \cite{Laskowski}, we have a formula $\theta(x, y)$, some $\A \subseteq \B \in \K$ with $\theta(x, y) \vdash \Delta_A(x) \wedge \Delta_\B(x, y)$. We may also assume that $\theta(x, y)$ is a conjunction of formulas of the type $\Psi_{\B, \C}(x,y)$ and their negations.
