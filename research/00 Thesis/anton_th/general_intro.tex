...
\begin{Definition} \ 
  ...
  \item Suppose we have a sequence $(a_i)_{i \in \I}$ where $\I$ is an ordered index set.
    For a subsequence $\J \subset \I$ let $a_{\J}$ denote the tuple obtained by the concatenation of the sequence $(a_j)_{j \in \J}$
    (where the sequence is ordered using the order of $\I$).    
  ...
  \item If we use the same definition, but allow tuples $a_{J_1}, a_{J_2}$ to be concatenated in arbitrary order,
    then we obtain the definition a sequence that is \defn{totally indiscernible over $B$} (alternatively \defn{an indiscernible set}).
  \end{itemize}
\end{Definition}
...
\begin{Lemma} [see Lemma 5.1.3 in \cite{tent}]
  ...
  Suppose $B \subset M$.
  Fix a variable tuple $x$ and a collection of formulas $\Delta(x_1, \ldots, x_n)$ with $|x_i| = |x|$.
  ...
  \end{align*}
  Then there exists an infinite indiscernible sequence $(a_i')_{i \in \N}$ with
  \begin{align*}
...
\begin{Definition} \label{def_dp} \ 
  ...
  \item Define
    ...
    \end{align*}
  \item The \defn{dp-rank} of an $\aleph_1$-saturated structure $M$ is $\leq n$ if $\vcind(\phi) \leq n$ for all formulas $\phi$.
