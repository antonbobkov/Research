...
For $\Q_p$ in the language $\LLM$, the paper \cite{density} computes an upper bound for the vc-function to be $2n-1$, and it is not known whether this is optimal.
...
In this chapter we investigate a reduct of the field of $p$-adic numbers and
...
Using this result, we improve the bound for the vc-function, showing that in Leenknegt's structure $\vc(n) = n$ for each $n$.
...
In Section 1 we recall some basic facts about the theory of $p$-adic numbers.
Here we also introduce the reduct which we will be working with.
Section 2 sets up basic definitions and lemmas that will be needed for the proof.
We define trees and intervals and show how they help with vc-density calculations. 
Section 3 concludes the proof.
...
\begin{Theorem} [Macintyre '76, \cite{mac76}]
...
\begin{Theorem} [Denef '84, \cite{den84}]
