...
%Shelah-Spencer graphs and superflat graphs are both subclasses of NIP theories, extremely well behaved model theoretically.
...
More generally, we work with complete theories and their models.
...
\begin{Definition} \ 
  ...
  \item Suppose we have a structure $\MM$.
    A formula $\phi(x,y)$ is called \defn{unstable} in $\MM$ if for all natural numbers $n$
  ...
  \item A structure $\MM$ is \defn{stable} if all of its formulas are stable in $\MM$.
...
\begin{Definition}
  ...
  \end{align*}
  is referred to as the \defn{$A$-definable} subset of $M^{|x|}$ defined by $\phi$.
\end{Definition}
...
Types are one of the main objects of study in model theory.
\begin{Definition}
  Let $\MM$ be a structure, $B \subseteq M$.
  Also fix a tuple of variables $x$.
  \begin{itemize}
  \item \defn{A partial type over $B$} is a collection of formulas in the variables $x$ with parameters from $B$.
  \item A tuple $a \in M^{|x|}$ is called \defn{realization} of a partial type $p(x)$ if
    $\MM \models \phi(a)$ for all $\phi(x) \in p(x)$.
  \item A partial type $p(x)$ is \defn{consistent} if every finite subset $p(x)$ has a realization in $\MM$.
  \item Let $a \in M^{|x|}$ and $\Delta \subseteq \LL(B)$ be a collection of formulas $\delta(x)$.
    Define the \defn{$\Delta$-type of $a$ over $B$} to be the collection of formulas $\delta(x) \in \Delta$
...
\begin{Definition}
  ...
  A structure $\MM$ is called \defn{$\kappa$-saturated} if for all $B \subset M$ with $|B| < \kappa$
...
\begin{Definition} \ 
  \begin{itemize}
  \item Suppose we have a sequence $(a_i)_{i \in \I}$ where $\I$ is an ordered index set and $a_i \in M^m$ for all $i$.
    For a subset $\J \subset \I$ let $a(\J) \in M^{m|\J|}$ denote the tuple obtained by the concatenation of the sequence $(a_j)_{j \in \J}$
    (where $\J$ is ordered using the order induced by $\I$).    
  \item Suppose $\MM$ is a structure, $B \subseteq M$, and $\I$ is an ordered index set. 
    A sequence $(a_i)_{i \in \I}$ with each $a_i \in M^m$ is called \defn{indiscernible over $B$} (or \defn{$B$-indiscernible}) if
    for any two finite subsets $\J_1, \J_2 \subseteq \I$ of equal size we have 
    \begin{align*}
      \tp(a(\J_1)/B) = \tp(a(\J_2)/B).
    \end{align*}
  \item If we use the same definition, but allow tuples $a(\J_1), a(\J_2)$ to be concatenated in arbitrary order,
    then we obtain the definition of a sequence that is \defn{totally indiscernible over $B$} (alternatively called \defn{an indiscernible set}).
  \end{itemize}
\end{Definition}

Here is an important property of indiscernible sequences in stable theories:
\begin{Lemma}[see Lemma 9.1.1 in \cite{tent}] \label{totally}
  If $\MM$ is stable then every indiscernible sequence (over a subset of $M$) is totally indiscernible.
\end{Lemma}

Instead of starting with an indiscernible sequence, we sometimes wish to construct one from a sequence with some degree of symmetry:
\begin{Lemma} [see Lemma 5.1.3 in \cite{tent}] \label{make_ind}
  Work in a $\kappa$-saturated structure $\MM$.
  Let $B \subset M$ with $|B| < \kappa$.
  Fix a tuple of variables $x$ and a collection of $B$-formulas $\Delta(x_1, \ldots, x_n)$ with $|x_i| = |x|$.
  Suppose we can find an arbitrarily long finite sequence $(a_i)_{i \in \I}$ with $a_i \in M^{|x|}$ such that
  for any subset $\J \subseteq \I$ of length $n$ we have
  \begin{align*}
    \MM \models \Delta(a(\J)).
  \end{align*}
  Then there exists an infinite $B$-indiscernible sequence $(b_i)_{i \in \N}$ such that
  for any subset $\J \subseteq \N$ of length $n$ we have
  \begin{align*}
    \MM \models \Delta(b(\J)).
  \end{align*}
\end{Lemma}
...
\begin{Definition}
  ...
  An \defn{automorphism} of $\MM$ over $A$ is a bijection $f \colon M \arr M$
...
\begin{Definition} \label{def_dp} \ 
  ...
  \item Define
    ...
    \end{align*}
  \item The \defn{dp-rank} of an $\aleph_1$-saturated structure $\MM$ is $\leq n$ if $\vcind(\phi) \leq n$ for all formulas $\phi$.
