...
\begin{Definition}
  ...
  \begin{enumerate}
  \item $G' = G[V]$ is called \defn{induced} subgraph of $G$ \defn{spanned} by $V$ if it is obtained as a subgraph of $G$ by taking all edges between vertices in $V$.
  ...
  \item We say that $A$ \defn{separates} $V$ if for all $a,b \in V$, $d_A(a,b) = \infty$.
  \item We say that $V$ has \defn{connectivity} $n$ if there are no sets of size $n-1$ in $V(G)$ that separate $V$.
  \item Suppose $V$ has finite connectivity $n$. \defn{Connectivity hull} of $V$ is defined to be the union of all sets separating $V$ of size $n-1$.
  \end{enumerate}
\end{Definition}
...
\begin{Definition}
  ...
  A graph is called \defn{superflat} if for every $m \in \N$ there is $n \in \N$ such that the graph avoids $K^m_n$ as a subgraph. 
\end{Definition}
