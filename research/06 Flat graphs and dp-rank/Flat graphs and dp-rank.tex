...
\begin{Lemma}
	Let $I = (a_i : i \in \I)$ be a countable indiscernible sequence over $A$. Fix $n \in \N$. There exists a finite set $B$ such that
	\begin{align*}
		\forall i \neq j \ d_B(a_i, a_j) > n
	\end{align*}
\end{Lemma}
...
\begin{proof}
	By a flatness result we can find an infinite $\J \subset \I$ and a finite set $B'$ such that each pair from $J = (a_j)_{j \in \J}$ have distance $>n$ over $B'$. Using total indiscernibility we have an automorphism sending $J$ to $I$. Image of $B'$ under this automorphism is the required set $B$.
\end{proof}

In other words, $B$ disconnects $P_n((a_i)_{i \in I})$.
This shows that $(a_i)_{i \in I}$ has finite connectivity in $P((a_i)_{i \in I})$.
Applying Corollary \ref{cr_hull_finite} we obtain that connectivity hull of $\{a_i\}$ in $P_n(\{a_i\})$ is finite.
