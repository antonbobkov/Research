...
\begin{enumerate}
	\item Let $A \subset V(G)$ with $G$ an infinite graph. Then for $a,b \in V(G)$ define $d_A(a,b)$ to be the distance (length of the smallest path) in an induced subgraph of $G$ after removing veritces $A$.
	\item Let $A, V \subset V(G)$. We say that $A$ separates $V$ if for all $a,b \in V$, $d_A(a,b) = \infty$.
\end{enumerate}
