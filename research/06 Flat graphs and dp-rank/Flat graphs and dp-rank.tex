...
\begin{proof}
	...
	To every $V_i$ we associate two values. $n_i \in \N$ is connectivity of $V_i$ and $S_i$ is its connectivity hull, which is finite by previous lemma. For $i \leq j$ we have $n_i \leq n_j \leq n$. If $n_i = n_j$, then $S_i \supseteq S_j$
\end{proof}
