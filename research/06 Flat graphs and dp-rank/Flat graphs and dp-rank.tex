...
\begin{enumerate}
	\item Let $A \subset V(G)$ with $G$ an infinite graph. Then for $a,b \in V(G)$ define $d_A(a,b)$ to be the \emph{distance} (length of the smallest path) in an induced subgraph of $G$ after removing veritces $A$.
	\item Let $A, V \subset V(G)$. We say that $A$ \emph{separates} $V$ if for all $a,b \in V$, $d_A(a,b) = \infty$.
	\item We say that $V$ has c
\end{enumerate}
