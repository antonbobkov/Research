...
\begin{Lemma}
	Fix an indiscernible sequence $I = (a_i : i \in \I)$ and let $H$ be its connectivity hull inside of graph $P_n(I)$. Then there are $N \in \N$ and a formula $\phi(x, y)$ with $|x| = 1$ and $|y| = N$ such that for every $J \subset I$ with $|J| = N$ we have
	\begin{align*}
		\phi(G, J) = H
	\end{align*}
