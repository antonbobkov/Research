...
\begin{Definition}
	...
	\begin{itemize}
		\item $\dim \paren{M_i/M_{i-1}} = -\epsilon^\phi_i$.
		\item $\epsilon_L(\phi) = \sum_{[1..k]} \epsilon^\phi_i$.
		\item $\epsilon^\phi_U = \min_{[1..k]} \epsilon^\phi_i$.
...
\begin{Definition}
	\epsilon^{\neg \phi}_L &= \epsilon^\phi_L \\
	\epsilon^{\neg \phi}_U &= \epsilon^\phi_U \\
	\epsilon^{\phi \wedge \psi}_U &= \epsilon^\phi_U \\
\end{Definition}
