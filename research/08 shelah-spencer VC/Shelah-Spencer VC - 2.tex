...
First, consider a case of a single basic formula $\phi$
...
Work with a parameter set $A^{|x|}$, with $A \subset \text{\textit{ambient model}}$.

% In general we have A \subset M^{|x|}

Pick a trace of $\phi(x,y)$ on $A$ by a parameter $b$, that is
\begin{align*}
