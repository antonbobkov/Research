...
\begin{Theorem}
	If for $b_1$ and $b_2$, $I_{b_1} = I_{b_2}$ and $\partial_{b_1} = \partial_{b_2}$ then they define the same trace $A_{b_1} = A_{b_2}$.
\end{Theorem}

Thus to bound the number of traces it is sufficient to bound the number of possible $I_b$ and $\partial_b$.
