...
Take $A' = \bigcup B' = \subset A$.
Note that $\bigcup B' = \bigcup B$, so all elements in $B$ have to have components from some elements of $B'$.
$|A'| \leq n|x|$, thus the size of $B$ is at most $(n|x|)^|x|$.

Suppose we have $C \subset A^{|x|}$, finite with $|C| = N$.
Then the underlying set $C' = \bigcup C$ has size at most $N|x|$.
Suppose we pick a subset $B \subset C$ with underlying set $B' = \bigcup B$ with size at most $n|x|$.
How many such choices are there for $B$?
There are ${N \choose n}$

Given set $|A| = N$ how many subsets of $B \subset A^{|x|}$ can we pick such that with $A' = \bigcup B$ we have $|A'| \leq n|x|$?
${N \choose n}$
