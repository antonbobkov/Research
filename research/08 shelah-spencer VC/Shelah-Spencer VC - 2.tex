...
Suppose we have a set $B$ and a minimal pair $(M, A)$ with $A \subset M$ and $\dim(M/A) = -\epsilon$. Then either $M \subseteq B$ or $\dim((M \cup B)/B) \leq -\epsilon$.
...
Suppose we have a set $B$ and a minimal chain $M_n$ with $M_0 \subset B$ and dimensions $-\epsilon_i$. Then either $M_n \subseteq B$ or $\dim((M_n \cup B)/B) \leq -\epsilon$.
