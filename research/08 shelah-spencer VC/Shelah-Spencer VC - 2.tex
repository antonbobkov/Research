...
\begin{Definition}
	...
	Call $f \colon G_1 \arr G_2$ a $\partial$-isomorphism if it is a graph isomorphism and it is constant on $\bar A$.
	For each $b$ define $\II_b$ as the $\partial$-isomorphism class of the graph induced on vertices $(\bar M_b - \bar A) \cup \partial_b$. %depends on witness choice
\end{Definition}
...
\begin{proof}
	...
	This gives us a witness $M_2$ via an isomorphism from $\bar M_{b_1} - \bar A$ to $\bar M_{b_2} - \bar A$.
\end{proof}

Thus to bound the number of traces it is sufficient to bound the number of possibilities for $\II_b$ and $\partial_b$.
