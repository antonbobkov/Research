...
\begin{Definition}
	...
	\begin{itemize}
		\item $\phi(x, y)$ is a minimal chain extension.
		Denoting that chain  by $\curly{M_i}_{i \in [0..k]}$ we also require that $M_0 = \{x, y\}$
...
\begin{Definition}[Conjunction]
	...
	If different formulas define graphs for $x$ differently then $\epsilon_L(\phi) = \infty$.
	% upper bound - how are the $x$ connected?
	Otherwise
	\begin{align*}
...
\begin{proof}
	...
	Finally consider the case when the chose conjunction consists of negative basic formulas
	\begin{align*}
		\psi = \bigwedge \neg \phi_i
	\end{align*}
	Let
...
We compute an upper bound on a collection of formulas $\{\phi_i\}$. This would automatically bound any boolean combinations of such formulas. 
...
\begin{Definition}
	Suppose $\phi$ is in positive form.
	A \emph{witness} of $\phi(a_i, b)$ is the union of realizations of all the positive existential formulas.
\end{Definition}
