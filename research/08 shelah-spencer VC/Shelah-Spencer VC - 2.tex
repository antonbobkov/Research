...
\begin{Definition}
	...
	\begin{itemize}
		...
		\item Call $f \colon G_1 \arr G_2$ a boundary-isomorphism if it is a graph isomorphism and it is constant on $\bar A$.
		$\II_b$ is the boundary-isomorphism class of the graph induced on vertices $(\bar M_b - \bar A) \cup \partial_b$.
	\end{itemize}
\end{Definition}
...
\begin{proof}
	...
	Pick its witness $M_1 \subset M_{b_1}$.
...
\begin{Theorem} \label{main_bound}
	\begin{align*}
		\text { any graph in $\II_b$ has size} &\leq K(\phi) \frac{Y(\phi)}{\epsilon(\phi)}\\
