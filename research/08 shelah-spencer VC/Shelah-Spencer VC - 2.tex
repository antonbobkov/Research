...
We bound the number of types of some finite collection of formulas $\Psi(\vec x, \vec y)$ over a parameter set $B$ of size $N$.
...
Let $S = |\vec y| \cdot \min \floor{\frac{K(\phi)}{\epsilon(\phi)}}$ (dependent only on $\phi$).
