...
\begin{proof}
	Proceed by induction.
	Second proposition is clear.
	For the first proposition base case is clear.
	
	For induction step, if $\bar M_j = \bar M_{j+1}$ then $m_i = m_{i+1}$ and the quantities don't change.
	Thus assume $\bar M_j \neq \bar M_{j+1}$.
	...
	\begin{itemize}
		...
		\item $M_{j+1} \subset \bar M_j \cup A$. Our assumption implies $M_{j+1} \cap A \nsubseteq \bar M_j$.
		There are edges between $M_{j+1} \cap A$ and the set (WHY?) so they contribute some negative dimension.
	\end{itemize}
\end{proof}
...
\begin{align*}
	a \leq m \cdot |M| \leq W
\end{align*}

Now, classify every trace by the isomorphism class of $\bar M - A$ and by $\bar M_j \cap A$.
\begin{Lemma}
...
\begin{align*}
	W \cdot 2^{W\cdot W} \cdot {|A| \choose W} = O(|A|^W)
\end{align*}
