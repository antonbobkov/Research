...
\begin{proof}
	...
	\begin{proof}
		...
		Let $B_i = B \cap M_i \subseteq M_i$.
		Then $B_i$'s are disjoint over $\bar A = A \cup b$.
		In particular $\dim B / \bar A = \sum \dim B_i / \bar A$.
		$\dim B_i / \bar A$ \geq -\epsilon_L(\phi)& by Lemma \ref{chain_intersect}.
		%If all $B_i = M_i$, then $B = \bar M$ and its dimension is 
...
\begin{Lemma} \label{chain_intersect}
	...
	Suppose we have some $B$ with $M_0 \subseteq B \subseteq M_n$.
	Then $\dim B / M_0 \geq -\epsilon$
\end{Lemma}
