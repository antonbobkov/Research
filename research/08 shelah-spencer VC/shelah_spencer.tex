...
\begin{Definition} \ 
  ...
  \item Given $S \in \N$, a graph $S \subseteq \DB$ is called \defn{$S$-strong}
    if for any $R \subseteq T \subseteq \DB$ with $|T| - |R| \leq S$ we have $R \leq T$.
  ...
  %   So for a graph $\DD$ and a tuple $b = (b_1, \ldots, b_n) \in \DD^n$ we have $\DD \models \diag_A(b)$ if and only if
  %   there exists an embedding $f \colon A \arr \DD$ such that $f(a_i) = b_i$ for each $i$.
...
\begin{Theorem} [Quantifier simplication, 5.6 in \cite{laskowski}]
...
\begin{Definition} \label{def_basic}
  ...
  where $W$ is a witness of $\phi$.% over $X \cup Y$.
\end{Definition}
