...
From the diagram we see that $\delta(\B/\A) \leq \delta(\B_1 \cup \A/\A) + \delta\left((\B_2 \cup \A)/((\B_1 \cap \B_2) \cup \A)\right)$. Thus all we need to do is to verify that $\delta\left((\B_2 \cup \A)/((\B_1 \cap \B_2) \cup \A)\right) \leq $
