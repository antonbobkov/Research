...
\begin{Definition}
	\begin{enumerate}
		\item Given an initial segment $S$ let $S^* = I_{M'}(S)$, initial closure of $S$ in the monster model.
		\item Given an initial segment $S$ let $S' = S_{M'}(F_A(S))$, initial part of 
	\end{enumerate}
\end{Definition}
