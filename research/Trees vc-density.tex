...
In this section all the considered trees $T$ are stable.  which are exactly trees of finite height.

\begin{Definition}
Given a tree $T$ of finite height and $n \leq m$ with $n \in \N, m \in \N \cup \{\infty\}$. Define $T \midr [n,m]$ to be the subtree of $T$ obtained restricting nodes at depth $[n,m]$. Call $T \midr [n, \infty]$ the \emph{$n$-slice} of $T$.
\end{Definition}

\begin{Definition}
	Consider a tree $T$ of height $N$. Let $S_n$ denote the collection of connected components in the $n$-slice of $T$. $T$ is called \emph{almost finite} if for every $n \leq N$, $S_n$ contains finitely many elements up to a (poset) isomorphism.
\end{Definition}
