...
\begin{Proof}
	Note that in almost finite theory all slices are also almost finite. Prove the statement by the reverse induction on slices' depth. Suppose we have proven the claim for $n$th slice. Consider a node $t \in T$ at depth $n-1$. All the cones under $t$ has different theories by induction, and there are finitely many isomorphism classes of them by assumption that $T$ is almost finite. Label those isomorphism classes $1$ through $N$. We assign $t$ a code which is a sequence of length $N$ with elements of $\N \cup \{\infty\}$. This is done by counting number of occuring isomorphic cones. $i$th element of the sequence is going to be the number of cones that have $i$th isomorphism class. Clearly two nodes have the same isomorphism class if and only if they have the same code. Two different codes can be told apart by their theories as all the children nodes have theories that can be told apart by a sentence.
\end{Proof}
