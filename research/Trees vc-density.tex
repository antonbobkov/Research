...
\begin{Proof}
	Consider a node $t \in T$. All the cones under $t$ has different theories by induction, and there are finitely many isomorphism classes of them by assumption that $T$ is almost finite. Label those isomorphism classes $1$ through $N$. We assign $t$ a code which is a sequence of length $N$ with elements of $\N \cup \{\infty\}$. This is done by counting number of occuring isomorphic cones. $i$th element of the sequence is going to be the number of cones that have $i$th isomorphism class. Clearly two nodes have the same isomorphism class if and only if they have the same code. Two different codes can be told apart by their theories as all the children nodes have theories that can be told apart by a sentence.
\end{Proof}

\begin{Definition}
	Given an element $t \in T$ in a tree, let $A_t = \{a \in T \mid a > t\}$ denote all the elements below $t$. Call a connected component of $A_t$ a \emph{child cone} of $t$.
\end{Definition}
...
\begin{proof}
	...
	$(2) \Rightarrow (3)$. We prove this by contrapositive. Suppose $T$ is not almost finite. Pick largest $n$ such that $n$-slice has infinitely many isomorphism classes. Thus $(n+1)$-slice is almost finite. Label them $0$ through $N$. Now, every element of $S_n$ can be coded by a sequence of elements of $\N \cup \{\infty\}$ of length $N$ as follows. Pick $s \in S_n$ and $k \in [N]$. Let $c \in S_{n+1}$ be coded by $k$. Count number of copies of $c$ as child cones of $s$ and record that number as $k$th element of the coding sequence. Two elements of $S_n$ will have the same coding sequence iff they are isomorphic. There are infinitely many isomorphism classes, each coded by a finite sequence. This implies that there has to be some $k$ such that $k$th position has arbitrarily high finite values. This means 
\end{proof}
