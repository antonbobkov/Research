...
\begin{Lemma}
	Suppose $T$ is almost finite. Then all the non-isomorphic connected components have different theories.
\end{Lemma}

\begin{Proof}
	Consider a node $t \in T$. All the cones under $t$ has different theories by induction, and there are finitely many isomorphism classes of them by assumption that $T$ is almost finite. Label those isomorphism classes $1$ through $N$. We assign $t$ a code which is a sequence of length $N$ of elements of $\N \cup \{\infty\}$
\end{Proof}
