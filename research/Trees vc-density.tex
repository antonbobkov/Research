...
\begin{Definition}
	Consider a tree $T$ of height $N$. Let $S_n$ denote the collection of connected components in the $n$-slice of $T$. $T$ is called \emph{almost finite} if for every $n \leq N$, $S_n$ contains finitely many element up to a (tree) isomorphism.
\end{Definition}
...
\begin{enumerate}
	\item $T$ is $\kappa_0$-categorical
	\item $T$ has NFCP
	\item $T$ is almost finite
	\item up to isomorphism every element $t \in T$ has finitely many children cones
\end{enumerate}
\end{Theorem}
\end{document}