...
\begin{proof}
	...
	\begin{align*}
		%P(t) \iff \{s \text{ node under $t$} \midr T \models \psi_i(s)\} \text { has $n_a$ elements}
		N(t) = \left|\{s \text{ node under $t$} \midr A_s \text { has isomorphism class $C$} \}\right|
	\end{align*}
	can take arbitrarily large values depending on $t$.
	Applying lemma \ref{lm_categoricity} to $T_{n+1}$.
