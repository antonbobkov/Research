...
We work with finite relational languages. Given a formula we can define its complexity $n$ as the depth of quantifiers used to build up the formula. See for example \cite{ynm_notes} Definition 2D.4 pg.72. We will use the following notation
\begin{Definition}
	\begin{itemize}
		\item Let $n$ be naturals, $\B$ a structure and $a$ a tuple in $\B$. Then $\tp^n_{\B}(a)$ will stand for the type
