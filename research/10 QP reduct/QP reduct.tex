...
We work with the reduct of p-adic numbers in the language $\LL_R = \curly{\Q_p, \curly{Q_{n,m}}_{n,m\in \N}, +, -, \curly{\bar c}_{c \in K} }$,
...
\begin{Lemma}
	...
	Moreover we have that if $a, a' \in Q_p^{|x|}$ agree on all formulas from $\Psi'$ then they agree on $\phi$.
\end{Lemma}

\begin{proof}
	To see that apply cell decomposition theorem to $\phi(t, \bar x)$.
	Let $q_i$ enumerate all of the following polynomials $a_1(\bar x), a_2(\bar x), t - c(\bar x)$ that show up in the cells
	Let $D_l$ be the smaller cells for $\bar x$ component that appear in the cells.
	Choose $n,m$ large enough to cover all $n', m'$ that come up in the cells for $Q_{n',m'}$.
	Choose $\lambda_k$ to go over all cosets of $Q_{n,m}$.
\end{proof}

Applying this lemma inductively to smaller cells that appear we obtain a family $\Psi(x)$
\begin{align*}
		&\val \paren{q_i(x)} < \val \paren{q_j(x)} & i, j \in I \\
		&\val \paren{q_i(x)} \in \lambda_k Q_{n,m} & i \in I , k \in K
\end{align*}
...
$\lambda_k \in \Q_p$, and
$Q = Q_{n,m}$ for some $n,m$.
Moreover whenever $a, a' \in Q_p^{|x|}$ agree on all formulas from $\Psi$ then they agree on $\phi$.
...
$\lambda_k \in \Q_p$, and
$Q = Q_{n,m}$ for some $n,m$
(to do this we simply split polynomials $q_i$ into its $x$ part and into its $y$ part including the constant term).
Now for any parameter set $B$ we have that if $a, a'$ have the same $\Psi$-type then they have the same $\phi$-type.
...
\begin{Definition}
	$a, a' \in \Q_p^{|x|}$ have the same $\Psi$-type if they have the same $\Psi$ type over $B$.	
\end{Definition}
...
\begin{proof}
	...
	$\Bt$ records interval type of each element, so if $g(\bar a) = g(\bar a')$ then $a_i, a_i'$ have the same interval type for all $i \in I$.
	Thus it remains to show that $a_i, a_i'$ lie in the same interval for all $i \in I$.
	Suppose $i$ is an independent index.
	Then by construction, $\Pt$ records interval for $a_i, a_i'$, so those have to belong to the same interval.
	...
	\begin{proof}
		...
		as needed.
	\end{proof}	
	moreover because $a_i, a_i'$ have the same image in $\Ct$component under $g$ we can conclude that
	\begin{align*}
		\val(a_i - a_i')) > F(a_j) 
	\end{align*}
...
\begin{proof}
	...
	Thus the number of possible $\Psi$-types is bound by the size of the range of $g$ times the number of possible partitions.
	...
	Thus we have $|J| \leq |x|$ as needed.
\end{proof}

\begin{Corollary}
	In the language $\LL_R$ we have $\vc(n) = n$.
\end{Corollary}
