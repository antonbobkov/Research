...
\begin{proof}
	...
	\end{Claim}
	\begin{proof}
		Let $\vec x, \vec x' \in \Q_p^m$ be some elements with
		\begin{align*}
...
\begin{Corollary}
	$\Psi(x,y)$ has dual VC-density $\leq |x|$
\end{Corollary}

\begin{proof}
	Suppose we have $c, c' \in \Q_p^m$ such that $f(c), f(c')$ are in the same partition and $g(f(c)) = g(f(c'))$.
