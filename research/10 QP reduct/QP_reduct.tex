...
\begin{Definition}
  ...
  We say that $a$ is \defn{far from the boundary} (of $\interval$) if 
    ...
    \end{align*}
  Here $n$ is from the Lemma \ref{quantifier_elimination}.
...
\begin{Definition}
  ...
  \item Both $a_1, a_2$ are far from the boundary and $a_1 - t, a_2 - t$ are in the same $Q_{m,n}$-coset.
    ($Q_{m,n}$ is from the Lemma \ref{quantifier_elimination}.)
...
\begin{Definition}
	For $c \in \Q_p$ and $\alpha, \beta \in \Z, \alpha < \beta$ define $c \midr [\alpha, \beta) \in \paren{\Z/p\Z}^{\beta - \alpha}$
...
\begin{proof}
  ...
  The number of such subinterval types is bounded by the number of $Q_{m,n}$-cosets.
  ...
  Such a subinterval type is thus determined by $\tval(a) - \alpha_L$ and $a \midr [\tval(a), \tval(a) + n)$,
...
\begin{Corollary}	 \label{gamma}
  ...
  Suppose $J \subset I$ and $i \in I$ satisfy $\vec p_i \in \vecspan \curly{\vec p_j}_{j \in J}$,
  and we have $\vec c \in \Q_p^{|x|}, \alpha \in \Z$ with $\val(\vec p_j \cdot \vec c) > \alpha \text{ for all } j \in J$.
...
\begin{Definition}
  ...
  Define \defn{$T$-floor} of $a$ to be $F(a) = \alpha_L$.
\end{Definition}
...
look at the corresponding $T$-floors $\curly{F(a_i)}_{i\in I}$ and T-valuations $\curly{\tval(a_i)}_{i\in I}$.
...
\begin{align*}
  F(a_1) \geq F(a_2) \geq \ldots \text { for all $a_i \in \Sg'$}
\end{align*}
...
\begin{proof}
  ...
  \begin{proof}
    ...
    \begin{align*}
      ...
      &\val(\vec p_k \cdot \vec c - \vec p_k \cdot \pvec c') > F(a_j) \text { for all } k \in K \\
      &\val(\vec p_k \cdot (\vec c - \pvec {c}')) > F(a_j) \text { for all } k \in K \\
    \end{align*}
    $K \subset I, i \in I, \vec c - \pvec {c}' \in \Q_p^{|x|}, F(a_j) \in \Z$
    satisfy the requirements of Lemma \ref {gamma}, so we apply it to conclude
    \begin{align*}
      &\val(\vec p_i \cdot (\vec c - \pvec {c}')) > F(a_j) - \gamma \\
      &\val(\vec p_i \cdot \vec c - \vec p_i \cdot \pvec {c}') > F(a_j) - \gamma \\
      &\val(a_i - a_i') > F(a_j) - \gamma
    \end{align*}
...
\begin{Corollary} [Theorem \ref{main_theorem}]
  $\LL_{aff}$-structure $\Q_p$ has $\vc(n) = n$.
\end{Corollary}
