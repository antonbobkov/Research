...
Group the terms
\begin{align*}
  f(\w) &= h(\w) + f_\gamma \w^{a_\gamma} + f^*(\w) + f_\beta \w^{a_\beta} \\
  g(\w) &= h(\w) + g_\gamma \w^{a_\gamma} + g^*(\w) + g_\beta \w^{a_\beta}
\end{align*}
where
\begin{align*}
	h(\w) &= f(\w)\midr\gamma = g(\w)\midr\gamma \\
	f^*(\w) &= f(\w)\midr\beta - f(\w)\midr{\gamma + 1} \\
	g^*(\w) &= g(\w)\midr\beta - g(\w)\midr{\gamma + 1}
\end{align*}

where $h(x)$ has the powers $x^{a_i}$ for $i < \gamma$ and $f*(x), g*(x)$ have the powers $x^{a_i}$ for $\gamma < i < \beta$.
Then we have by tail property $f*(\omega) << x^{a_\gamma}$ and $g*(\omega) << x^{a_\gamma}$. Compute

\begin{align*}
  f(\w) - g(\w) &= (f_\gamma - g_\gamma) x^{a_\gamma} + (f*(\w) - g*(\w)) + (f_\beta - g_\beta) x^{a_\beta}
\end{align*}

Precompute 

\begin{align*}
  f(\w)\midr\beta - g(\w)\midr\beta &= 
	\brac{f(\w)\midr\beta - f(\w)\midr{\gamma + 1} } - 
	\brac{g(\w)\midr\beta - g(\w)\midr{\gamma + 1} } +
	\brac{f(\w)\midr{\gamma + 1} - g(\w)\midr{\gamma + 1} }\\
	&= \brac{<< \w^{a_\gamma}} + \brac{<< \w^{a_\gamma}} +
	\brac{f(\w)\midr{\gamma + 1} - g(\w)\midr{\gamma + 1} } \\
	&= \brac{<< \w^{a_\gamma}} + \brac{f(\w)\midr\gamma - g(\w)\midr\gamma } +
	\brac{(f_\gamma - g_\gamma) x^{a_\gamma}} \\
	&= \brac{<< \w^{a_\gamma}} + \brac{ 0 } + \brac{(f_\gamma - g_\gamma) x^{a_\gamma}} \\
	&= \brac{<< \w^{a_\gamma}} + \brac{(f_\gamma - g_\gamma) x^{a_\gamma}}	
\end{align*}

Compute

\begin{align*}
  f(\w) - g(\w) &= \brac{f(\w)\midr\beta - g(\w)\midr\beta} + \brac{f_\beta - g_\beta} \w^{a_\beta} \\
	&= \brac{f(\w)\midr\beta - g(\w)\midr\beta} + \brac{<< \w^{a_\gamma}} \\
	&= \brac{(f_\gamma - g_\gamma) x^{a_\gamma}} + \brac{<< \w^{a_\gamma}} > 0
\end{align*}

Both $(f*(\w) - g*(\w))$ and $x^{a_\beta}$ are dominated by $x^{a_\gamma}$ and $(f_\gamma - g_\gamma) \in \R^{>0}$.
...
\begin{align*}
  \bar f(x) &= h(x) + f_\gamma x^{a_\gamma} + f*(x)\\
  \bar g(x) &= h(x) + g_\gamma x^{a_\gamma} + g*(x)
\end{align*}
where $h(x)$ has the powers $x^{a_i}$ for $i < \gamma$ and $f*(x), g*(x)$ have the powers $x^{a_i}$ for $\gamma < i$. As $f_\gamma < g_\gamma$ we have $\bar f(x) < \bar g(x)$ and thus by induction hypothesis $\bar f(\w) < \bar g(\w)$. As choice of those was arbitrary we have $R_f < L_g$ so $f(\w) < g(\w)$.
