...
We also need to check that the function is well-defined. For $f(x) = \sum_{i < \alpha} f_i \w^{a_i}$ define its reduced form, where we only keep non-zero coefficients. $\bar f(x) = \sum_{j < \alpha'} f_i \w^{a_i}$
