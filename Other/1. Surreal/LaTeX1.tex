...
If $\alpha$ is arbitrary, so $\alpha = \alpha' + n$ where $\alpha'$ is a limit ordinal and $n \in \N$, we proceed by induction on $n$.
For simplicity assume that $\alpha$ is a limit and we want to show the result for $\alpha + 1$.
...
\begin{align*}
	...
	\curly{(f_{\alpha + 1} - \epsilon) \w^{a_{\alpha + 1}} \mid \ldots} \text{ (by (6.4) and induction hypothesis)} \\
\end{align*}
